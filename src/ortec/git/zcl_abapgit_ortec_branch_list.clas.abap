...
CLASS zcl_abapgit_ortec_branch_list IMPLEMENTATION.
  ...
  METHOD fill_last_commit_dates.
    ...
    IF mt_tip_commits IS INITIAL.
      ...
      lv_is_fastpath =
        zcl_abapgit_ortec_git_switch=>is_active_for_repo( mv_url ).
  ...
  METHOD classify_branch.
    ...
    IF lv_lower = 'main' OR lv_lower = 'master' OR lv_lower = 'develop' OR
       lv_lower = 'development' OR lv_lower = 'trunk' OR lv_lower = 'release'.
  ...
  METHOD render_filter_script.
    ...
    ri_html->add( '  });' ).
    ri_html->add( '' ).
    ri_html->add( '  // Drag-to-move' ).
    ri_html->add( '  (function() {' ).
    ri_html->add( '    var dlg = document.querySelector(".ortec-branch-picker");' ).
    ri_html->add( '    var th = document.getElementById("ortec-bp-titlebar");' ).
    ri_html->add( '    if (!dlg || !th) return;' ).
    ri_html->add( '    var dragging = false, sx, sy, sl, st;' ).
    ri_html->add( '    th.addEventListener("mousedown", function(e) {' ).
    ri_html->add( '      if (e.button !== 0) return;' ).
    ri_html->add( '      var r = dlg.getBoundingClientRect();' ).
    ri_html->add( '      dlg.style.transform = "none";' ).
    ri_html->add( '      dlg.style.left = r.left + "px";' ).
    ri_html->add( '      dlg.style.top = r.top + "px";' ).
    ri_html->add( '      dragging = true; sx = e.clientX; sy = e.clientY;' ).
    ri_html->add( '      sl = r.left; st = r.top;' ).
    ri_html->add( '      e.preventDefault();' ).
    ri_html->add( '    });' ).
    ri_html->add( '    document.addEventListener("mousemove", function(e) {' ).
    ri_html->add( '      if (!dragging) return;' ).
    ri_html->add( '      dlg.style.left = (sl + e.clientX - sx) + "px";' ).
    ri_html->add( '      dlg.style.top = (st + e.clientY - sy) + "px";' ).
    ri_html->add( '    });' ).
    ri_html->add( '    document.addEventListener("mouseup", function() { dragging = false; });' ).
    ri_html->add( '  })();' ).
  ...
  METHOD zif_abapgit_gui_renderable~render.
    ...
    ri_html->add( 'padding:0;width:760px;max-width:96vw;min-width:360px;' ).
    ri_html->add( 'height:80vh;min-height:200px;max-height:95vh;resize:both;' ).
    ri_html->add( 'position:fixed;left:50%;top:50%;z-index:10000;margin:0;' ).
    ri_html->add( 'transform:translate(-50%,-50%);' ).
    ...
    ri_html->add( '<div id="ortec-bp-titlebar" style="' ).
    ri_html->add( 'padding:10px 16px;background:#f6f8fa;border-bottom:1px solid #d0d7de;' ).
    ri_html->add( 'cursor:move;user-select:none;flex:0 0 auto;' ).
    ri_html->add( 'display:flex;align-items:center;border-radius:6px 6px 0 0;">' ).
    ri_html->add( '<span style="font-weight:600;font-size:14px;color:#2c3e50;">' ).
    ri_html->add( zif_abapgit_gui_page_title~get_page_title( ) ).
    ri_html->add( '</span>' ).
    ri_html->add( '</div>' ).
  ...
  METHOD zif_abapgit_gui_event_handler~on_event.
    ...
    CASE ii_event->mv_action.
      ...
      WHEN c_event-create.
        ...
        mv_fulfilled = abap_true.
        " In switch mode, perform_switch handles the create-branch flow
        IF mv_key IS NOT INITIAL.
          perform_switch( ms_result ).
        ENDIF.
        rs_handled-state = zcl_abapgit_gui=>c_event_state-go_back.
