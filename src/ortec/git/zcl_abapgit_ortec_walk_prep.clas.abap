CLASS zcl_abapgit_ortec_walk_prep DEFINITION
  ...
  PUBLIC SECTION.
    CLASS-METHODS prewarm
      ...
      RETURNING
        VALUE(rt_blob_sha1s) TYPE zif_abapgit_git_definitions=>ty_sha1_tt
      RAISING
    ...
    CLASS-METHODS fetch_blobs_bulk
      ...
      CHANGING
        ct_remaining_sha1s TYPE zif_abapgit_git_definitions=>ty_sha1_tt
      RETURNING
  ...
  PRIVATE SECTION.
    CLASS-METHODS warm_trees
      ...
      CHANGING
        ct_objects   TYPE zif_abapgit_definitions=>ty_objects_tt
      RETURNING
        VALUE(rt_tree_objects) TYPE zif_abapgit_definitions=>ty_objects_tt
      RAISING
        zcx_abapgit_ortec_git.

    CLASS-METHODS collect_blob_shas
      IMPORTING
        it_tree_objects TYPE zif_abapgit_definitions=>ty_objects_tt
      RETURNING
        VALUE(rt_blob_sha1s) TYPE zif_abapgit_git_definitions=>ty_sha1_tt
      RAISING
        zcx_abapgit_ortec_git.
...
CLASS zcl_abapgit_ortec_walk_prep IMPLEMENTATION.
  ...
  METHOD warm_trees.
    ...
    FIELD-SYMBOLS <ls_tree> LIKE LINE OF lt_tree_objects.
    ...
    WHILE lt_current_trees IS NOT INITIAL.
      ...
      LOOP AT lt_current_trees INTO lv_tree_sha.
        ...
        READ TABLE ct_objects WITH KEY type COMPONENTS
          type = zif_abapgit_git_definitions=>c_type-tree
  ...
  METHOD fetch_blobs_bulk.
    ...
    LOOP AT it_sha1s ASSIGNING <lv_sha1> WHERE table_line IS NOT INITIAL.
      ...
      APPEND <lv_sha1> TO lt_candidates.
      lv_total_bytes = lv_total_bytes + lv_this_bytes.
    ENDLOOP.
    ...
    LOOP AT lt_objects INTO ls_object.
      READ TABLE lt_candidates TRANSPORTING NO FIELDS WITH KEY table_line = ls_object-sha1.
      IF sy-subrc <> 0.
        CONTINUE.
      ENDIF.
      ...
      LOOP AT lt_nodes INTO ls_node.
        CASE ls_node-chmod.
          ...
          WHEN zif_abapgit_git_definitions=>c_chmod-file
            ...
            READ TABLE it_objects WITH KEY type COMPONENTS
