...
CLASS ltcl_cold_init IMPLEMENTATION.
  ...
  METHOD build_tip_fixture.
    ...
    zcl_abapgit_ortec_obj_store=>store_object(
      iv_repo_key = mc_repo iv_sha1 = ev_commit_sha
    ...
    zcl_abapgit_ortec_obj_store=>store_object(
      iv_repo_key = mc_repo iv_sha1 = ev_blob_sha
  ...
  METHOD all_present_needs_no_http.
    ...
    build_tip_fixture(
      IMPORTING ev_commit_sha = lv_commit_sha
    ...
    zcl_abapgit_ortec_mat_state=>mark_graph_complete(
      iv_repo_key = mc_repo iv_commit = lv_commit_sha iv_attempt_id = lv_attempt_id ).
  ...
  METHOD oversize_action_byte_limit.
    ...
    lv_action = zcl_abapgit_ortec_cold_init=>decide_oversize_action(
      ...
      iv_splits_used    = 0 ).
    cl_abap_unit_assert=>assert_equals(
      act = lv_action exp = zcl_abapgit_ortec_cold_init=>cs_oversize_action-none
    ...
    lv_action = zcl_abapgit_ortec_cold_init=>decide_oversize_action(
      ...
      iv_splits_used    = 0 ).
    cl_abap_unit_assert=>assert_equals(
      act = lv_action exp = zcl_abapgit_ortec_cold_init=>cs_oversize_action-split
    ...
    lv_action = zcl_abapgit_ortec_cold_init=>decide_oversize_action(
      ...
      iv_splits_used    = 0 ).
    cl_abap_unit_assert=>assert_equals(
      act = lv_action exp = zcl_abapgit_ortec_cold_init=>cs_oversize_action-raise
  ...
  METHOD verify_batch_extra_ignored.
    ...
    zcl_abapgit_ortec_obj_store=>store_object(
      iv_repo_key = mc_repo iv_sha1 = lv_extra_sha
      iv_type = zif_abapgit_git_definitions=>c_type-blob iv_data = lv_extra_data ).
  ...
  METHOD verify_batch_wrong_type.
    ...
    zcl_abapgit_ortec_obj_store=>store_object(
      iv_repo_key = mc_repo iv_sha1 = lv_sha1
      iv_type = zif_abapgit_git_definitions=>c_type-tree iv_data = lv_data ).
