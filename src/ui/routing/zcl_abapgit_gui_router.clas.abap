  ...
  METHOD git_services.
    ...
    CASE ii_event->mv_action.
      ...
      WHEN zif_abapgit_definitions=>c_action-git_branch_delete.             " GIT Delete remote branch
        ...
        rs_handled-state = zcl_abapgit_gui=>c_event_state-re_render.
      WHEN zif_abapgit_definitions=>c_action-git_branch_switch.             " GIT Switch branch (rich picker)
        rs_handled-page  = zcl_abapgit_gui_page_hoc=>create(
          iv_page_title      = 'Switch Branch'
          ii_child_component = zcl_abapgit_ortec_branch_list=>create_for_switch( iv_key = lv_key ) ).
        rs_handled-state = zcl_abapgit_gui=>c_event_state-new_page.
